...
class DocumentProcessor:
    ...
    def process_excel_file(self, file_path: str) -> List[Document]:
        ...
        try:
            ...
            for i, doc in enumerate(documents):
                ...
                # Add extraction method if not present
                if 'extraction_method' not in doc.metadata:
                    doc.metadata['extraction_method'] = 'unstructured'
                
                # Add file info
                doc.metadata['file_size'] = self.document_metadata[filename]['size']
                doc.metadata['last_modified'] = self.document_metadata[filename]['last_modified']
            
            logger.info(f"Successfully processed Excel file: {filename} - extracted {len(documents)} sheets/elements")
            return documents
        except Exception as e:
    ...
    def process_word_file(self, file_path: str) -> List[Document]:
        """Process a Word document and extract content"""
        if not WORD_SUPPORT:
            logger.error("Word support not available. Install python-docx2txt package.")
            return []
        ...
        try:
            # Try with Docx2txtLoader first
            try:
                loader = Docx2txtLoader(file_path)
                documents = loader.load()
            except Exception as e:
                logger.warning(f"Docx2txt extraction failed for Word file {filename}. Trying UnstructuredWordDocumentLoader. Error: {e}")
                
                # Try with UnstructuredWordDocumentLoader as backup
                loader = UnstructuredWordDocumentLoader(file_path, mode="elements")
                documents = loader.load()
            
            if not documents:
                logger.error(f"Could not extract content from Word file {filename}")
                return []
            ...
            logger.info(f"Successfully processed Word file: {filename} - extracted {len(documents)} pages/elements")
            return documents
        except Exception as e:
            error_msg = f"Error processing Word file {file_path}: {str(e)}"
            logger.error(error_msg)
            return []
    
    def process_powerpoint_file(self, file_path: str) -> List[Document]:
        """Process a PowerPoint presentation and extract content with image OCR support"""
        if not PPTX_SUPPORT:
            logger.error("PowerPoint support not available. Install unstructured package.")
            return []
        ...
        try:
            # First try standard extraction with UnstructuredPowerPointLoader
            loader = UnstructuredPowerPointLoader(file_path, mode="elements")
            documents = loader.load()
            
            # If OCR is available, also process images in the PowerPoint
            if OCR_AVAILABLE:
                try:
                    # Extract images and perform OCR
                    image_documents = self._extract_pptx_images_with_ocr(file_path)
                    if image_documents:
                        logger.info(f"Extracted text from {len(image_documents)} images in PowerPoint: {filename}")
                        documents.extend(image_documents)
                except Exception as img_err:
                    logger.warning(f"Error extracting images from PowerPoint {filename}: {str(img_err)}")
            
            if not documents:
                logger.error(f"Could not extract content from PowerPoint file {filename}")
                return []
            ...
            for i, doc in enumerate(documents):
                ...
                if 'page' not in doc.metadata and 'slide' not in doc.metadata:
                    doc.metadata['slide'] = i + 1
                
                # Add document info
                doc.metadata['doc_type'] = 'pptx'
                
                # Update extraction method if not present
                if 'extraction_method' not in doc.metadata:
                    doc.metadata['extraction_method'] = 'unstructured'
            ...
            logger.info(f"Successfully processed PowerPoint file: {filename} - extracted {len(documents)} slides/elements")
            return documents
        except Exception as e:
            error_msg = f"Error processing PowerPoint file {file_path}: {str(e)}"
            logger.error(error_msg)
            return []

    def _extract_pptx_images_with_ocr(self, file_path: str) -> List[Document]:
        """Extract images from PowerPoint and perform OCR on them"""
                ...
                for shape_num, shape in enumerate(slide.shapes, 1):
                    ...
                    if hasattr(shape, 'image'):
                        try:
                            ...
                            # Only add if there's meaningful text (more than just spaces or a few chars)
                            if text and len(text.strip()) > 10:
                                # Create document with extracted text
                                doc = Document(
                                    page_content=f"[Image Text] {text.strip()}",
                                    metadata={
                                        'source': filename,
                                        'slide': slide_num,
                                        'image_num': shape_num,
                            ...
                            logger.debug(f"Error processing image in slide {slide_num}: {str(img_err)}")
                            continue
            
            return image_documents
        except Exception as e:
            logger.warning(f"Error extracting images from PowerPoint: {str(e)}")
            return []

    def _process_with_ocr(self, file_path: str) -> List[Document]:
        """Process a PDF file using OCR when regular extraction fails"""
        if not OCR_AVAILABLE:
            return []
    ...
    def split_documents(self, documents: List[Document]) -> List[Document]:
        """Split documents into optimized chunks with semantic awareness"""
        if not documents:
            return []
        ...
        try:
            for doc in documents:
                ...
                # Try semantic splitting based on filetype/content
                file_ext = ""
                if hasattr(doc, 'metadata') and 'source' in doc.metadata:
                    file_ext = os.path.splitext(doc.metadata['source'])[1].lower()
                
                # Use header splitting for markdown or text files that contain headers
                if file_ext in ['.md', '.txt'] or '# ' in doc.page_content:
                    try:
                        # First split by headers
                        header_splits = self.md_header_splitter.split_text(doc.page_content)
                        
                        # Convert splits to Document objects with metadata
                        for split in header_splits:
                            ...
                            for header_key in ['Header 1', 'Header 2', 'Header 3']:
                                if header_key in split.metadata:
                                    metadata[header_key] = split.metadata[header_key]
                            ...
                            # Further split by length if needed
                        ...
                        logger.warning(f"Header splitting failed: {str(e)}. Falling back to recursive splitting.")
                        # Fall back to recursive splitting
                        doc_chunks = self.text_splitter.split_documents([doc])
                        chunked_docs.extend(doc_chunks)
                else:
                    # Use recursive splitting for other document types
                    doc_chunks = self.text_splitter.split_documents([doc])
                    chunked_docs.extend(doc_chunks)
            
            # Enhance metadata for all chunks
            for chunk in chunked_docs:
                # Ensure metadata is preserved from parent document
                if not hasattr(chunk, 'metadata') or not chunk.metadata:
                    ...
                    for original_doc in documents:
                        if original_doc.page_content in chunk.page_content:
                            chunk.metadata = original_doc.metadata.copy()
                            break
                
                # Add chunk-specific metadata
                chunk.metadata['chunk_size'] = len(chunk.page_content)
                
                # Add filename to beginning of content for better context
                source = chunk.metadata.get('source', '')
                if source and not chunk.page_content.startswith(f"Filename: {source}"):
                    chunk.page_content = f"Filename: {source}\n\n{chunk.page_content}"
    ...
    def get_document_count(self) -> Dict[str, int]:
        """Get counts of processed documents"""
        ...
        pdf_count = sum(1 for filename in processed_files 
                         if filename.lower().endswith('.pdf'))
        text_count = sum(1 for filename in processed_files 
                          if any(filename.lower().endswith(ext) for ext in ['.txt', '.md', '.csv', '.json']))
        excel_count = sum(1 for filename in processed_files 
                           if any(filename.lower().endswith(ext) for ext in ['.xlsx', '.xls']))
        word_count = sum(1 for filename in processed_files 
                          if any(filename.lower().endswith(ext) for ext in ['.docx', '.doc']))
        pptx_count = sum(1 for filename in processed_files 
                          if any(filename.lower().endswith(ext) for ext in ['.pptx', '.ppt']))
        sas_data_count = sum(1 for filename in processed_files 
                            if filename.lower().endswith('.sas7bdat'))
        sas_code_count = sum(1 for filename in processed_files 
                            if filename.lower().endswith('.sas'))
                
        # Get total chunks from vector store (approximate)
        ...
        return {
            ...
            "pptx_documents": pptx_count,
            "sas_data_documents": sas_data_count,
            "sas_code_documents": sas_code_count
        }
    
    def get_vector_store(self):
        """Get the vector store for search operations"""
        return self.vector_store
    
    def get_all_documents(self) -> List[Document]:
        """Get all processed document chunks (approximation from recent processing)"""
        # This is an approximation since we don't store all documents in memory anymore
        # For accurate search, use the vector_store methods directly
        if self.vector_store.vector_store and hasattr(self.vector_store.vector_store, 'docstore') and hasattr(self.vector_store.vector_store.docstore, '_dict'):
            return list(self.vector_store.vector_store.docstore._dict.values())
        return []
    
    def get_document_content(self) -> Dict[str, str]:
        """Get the full content of all processed documents"""
        return self.all_document_content
